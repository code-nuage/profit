...
return function(router)
    router:add_route("/register", "POST", function(req, res)
        if not middleware_connection(req) then
            local code, body, content_type, set_cookie = controller_connection.register(req.body)
    ...
    :add_route("/login", "POST", function(req, res)
        if not middleware_connection(req) then
    ...
    :add_route("/rename", "PATCH", function(req, res)
        res.status, res.body, res.header["Content-Type"] = controller_connection.rename(req.headers["cookie"], req.body)
    end)
    :add_route("/delete", "DELETE", function(req, res)
        res.status, res.body, res.header["Content-Type"] = controller_connection.delete(req.headers["cookie"])
    end)
    :add_route("/resetpassword", "PATCH", function(req, res)
        res.status, res.body, res.header["Content-Type"] = controller_connection.resetpassword(req.headers["cookie"], req.body)
    end)
